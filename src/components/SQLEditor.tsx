...
if (typeof window !== 'undefined') {
  loader.init().then((monaco: typeof MonacoType) => {
    monaco.editor.defineTheme('qsql-dark', {
      ...
      inherit: true,
      rules: [
        { token: 'keyword', foreground: '7c9aff', fontStyle: 'bold' },
      ...
      colors: {
        ...
        'editorCursor.foreground': '#7c9aff',
        'editor.lineHighlightBackground': '#151b2820',
        ...
        'editorBracketMatch.border': '#7c9aff30',
      },
    });
    ...
    monaco.editor.defineTheme('qsql-light', {
      ...
      colors: {
        ...
        'editor.selectionBackground': '#4338ca18',
        'editor.inactiveSelectionBackground': '#4338ca0c',
        'editorLineNumber.foreground': '#b0b8c4',
        'editorLineNumber.activeForeground': '#57606a',
        'editorGutter.background': '#fafbfc',
        'editorIndentGuide.background': '#e5e7eb',
        'editorIndentGuide.activeBackground': '#c9cdd3',
        'editorBracketMatch.background': '#4338ca10',
        'editorBracketMatch.border': '#4338ca30',
      },
    });
  });
}
...
export function SQLEditor({ tabId }: SQLEditorProps) {
  ...
  return (
    <div ref={containerRef} className="flex flex-col h-full overflow-hidden bg-background">
      ...
      <div className="flex items-center justify-between px-3 h-11 border-b border-border bg-card shrink-0">
        <div className="flex items-center gap-3">
          <button
            onClick={handleRun}
            disabled={running}
            className="flex items-center gap-2 px-4 py-1.5 rounded-lg bg-emerald-600 text-white font-semibold hover:bg-emerald-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 text-[13px] active:scale-[0.97] shadow-sm hover:shadow-md hover:shadow-emerald-500/15"
          >
            {running ? (
        ...
        <div className="flex items-center gap-1.5">
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all duration-200 ${showHistory ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:text-foreground hover:bg-muted/50'
              }`}
            title="Query History"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-muted-foreground hover:text-foreground hover:bg-muted/50 transition-all duration-200"
            title="Clear Editor"
          >
      ...
      {showHistory && (
        <div className="absolute right-3 top-[3.25rem] z-30 w-96 max-h-72 rounded-2xl overflow-hidden animate-scale-in elevated-card">
          ...
          <div className="overflow-y-auto max-h-56">
            {history.length === 0 ? (
              <div className="flex flex-col items-center py-8 text-muted-foreground">
                <History className="w-5 h-5 mb-2 opacity-30" />
              ...
              history.map((q, i) => (
                <button
                  ...
                  className="w-full text-left px-4 py-2.5 text-[11px] font-mono text-foreground hover:bg-primary/5 transition-all duration-150 border-b border-border/15 last:border-0 truncate group"
                >
                  <span className="text-muted-foreground/30 mr-2 text-[10px]">{i + 1}.</span>
      ...
      <div className="flex-1 flex flex-col min-h-0 relative">
        ...
        {hasResults && (
          <div
            className="h-[5px] cursor-row-resize flex items-center justify-center hover:bg-primary/8 transition-all duration-200 shrink-0 group relative"
            onMouseDown={handleDragStart}
          >
            <div className="absolute inset-x-0 -top-1.5 -bottom-1.5 z-10" />
            <div className="w-12 h-[3px] rounded-full bg-border/60 group-hover:bg-primary/50 group-hover:w-16 transition-all duration-200" />
          </div>
        )}
        ...
        {hasResults && (
          ...
          >
            ...
            <div className="flex items-center justify-between px-2.5 h-9 border-b border-border/30 bg-card/80 backdrop-blur-md shrink-0">
                ...
                {results.length > 1 &&
                  results.map((r, i) => (
                    <button
                      ...
                      className={`flex items-center gap-1 px-2 py-0.5 text-[11px] font-semibold rounded-md transition-all duration-200 ${i === activeResultIdx
            ...
            <div className="flex-1 overflow-auto">
              {activeResult?.error === EXEC_SQL_MISSING ? (
                <div className="p-5 space-y-4">
                  <div className="flex items-start gap-2.5 p-4 rounded-xl bg-amber-500/8 border border-amber-500/20">
                    <AlertCircle className="w-4 h-4 text-amber-500 shrink-0 mt-0.5" />
                    <div>
                      <p className="text-sm font-semibold text-foreground mb-1">Database function not found</p>
                      <p className="text-xs text-muted-foreground leading-relaxed">
                        QSQL requires a <code className="px-1.5 py-0.5 rounded-md bg-muted text-foreground text-[11px] font-mono">exec_sql</code> function in your Supabase database.
                  ...
                  {(() => {
                    ...
                    return (
                      <a
                        ...
                        className="btn-primary inline-flex items-center gap-1.5 px-4 py-2.5 rounded-xl bg-primary text-primary-foreground text-xs font-semibold hover:bg-primary/90 transition-colors shadow-sm"
                      >
                  ...
                  <div className="relative rounded-xl border border-border/40 bg-card overflow-hidden">
                    <div className="flex items-center justify-between px-3.5 py-2.5 border-b border-border/30 bg-muted/20">
                      ...
                      <button
                        ...
                        className="flex items-center gap-1 px-2 py-1 rounded-md text-[11px] font-medium text-muted-foreground hover:text-foreground hover:bg-muted/60 transition-all duration-200"
                      >
                        <Copy className="w-3 h-3" />
                        Copy
                      </button>
                    </div>
                    <pre className="p-3.5 text-[11px] font-mono text-foreground overflow-x-auto leading-relaxed max-h-64 overflow-y-auto">{EXEC_SQL_FUNCTION}</pre>
                  </div>
                ...
                <div className="p-5">
                  ...
                  <p className="mt-3 text-[11px] text-muted-foreground font-mono bg-muted/20 p-3 rounded-xl border border-border/20">
                    <span className="text-muted-foreground/50 mr-1">Query:</span> {activeResult.query}
                  </p>
                </div>
                ...
                (() => {
                  ...
                  if (isStatusResult) {
                    ...
                    return (
                      <div className="p-5">
                        <div className="flex items-start gap-2.5 p-4 rounded-xl bg-emerald-500/8 border border-emerald-500/20">
                        ...
                        <table className="w-full text-xs mt-4 rounded-xl border border-border/30 overflow-hidden">
                          <thead>
                            <tr className="border-b border-border/40 bg-muted/20">
                              <th className="text-left px-3.5 py-2 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Property</th>
                              <th className="text-left px-3.5 py-2 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider">Value</th>
                            </tr>
                          </thead>
                          <tbody>
                            <tr className="border-b border-border/15">
                              <td className="px-3.5 py-2 text-muted-foreground font-mono">status</td>
                              <td className="px-3.5 py-2 text-emerald-500 font-mono font-medium">success</td>
                            </tr>
                            <tr className="border-b border-border/15 bg-muted/5">
                              <td className="px-3.5 py-2 text-muted-foreground font-mono">message</td>
                              <td className="px-3.5 py-2 text-foreground font-mono">{String(row.message || '')}</td>
                            </tr>
                            {rowsAffected !== null && (
                              <tr className="border-b border-border/15">
                                <td className="px-3.5 py-2 text-muted-foreground font-mono">rows_affected</td>
                                <td className="px-3.5 py-2 font-mono"><span className="text-blue-500">{rowsAffected}</span></td>
                              </tr>
                            )}
                            <tr className={rowsAffected !== null ? 'bg-muted/5' : ''}>
                              <td className="px-3.5 py-2 text-muted-foreground font-mono">duration</td>
                              <td className="px-3.5 py-2 font-mono"><span className="text-blue-500">{activeResult.duration}</span><span className="text-muted-foreground">ms</span></td>
                            </tr>
                          </tbody>
                        </table>
                      </div>
                    ...
                    <table className="w-full text-xs">
                      <thead className="sticky top-0 z-10">
                        <tr className="border-b border-border/40 bg-card">
                          <th className="text-center px-2 py-2 text-[10px] font-semibold text-muted-foreground/40 bg-card w-10 border-r border-border/20">#</th>
                          {Object.keys(activeResult.data[0]).map((col) => (
                            <th
                              key={col}
                              className="text-left px-3.5 py-2 text-[11px] font-semibold text-muted-foreground uppercase tracking-wider whitespace-nowrap bg-card"
                            >
                              {col}
                            </th>
                          ))}
                      ...
                      <tbody>
                        {activeResult.data.map((row, rowIdx) => (
                          <tr
                            key={rowIdx}
                            className={`border-b border-border/15 last:border-0 hover:bg-primary/[0.03] transition-colors duration-150 ${rowIdx % 2 === 1 ? 'bg-muted/5' : ''
                              }`}
                          >
                            <td className="text-center px-2 py-1.5 text-[10px] text-muted-foreground/30 font-mono border-r border-border/15 select-none w-10">{rowIdx + 1}</td>
                            {Object.values(row).map((val, colIdx) => (
                              <td
                                key={colIdx}
                                className="px-3.5 py-1.5 text-foreground whitespace-nowrap font-mono text-xs"
                              >
                                {val === null ? (
                                  <span className="text-muted-foreground/40 italic text-[10px] px-1.5 py-0.5 bg-muted/20 rounded-md">NULL</span>
                                ) : typeof val === 'object' ? (
                ...
                <div className="flex flex-col items-center justify-center h-full py-10 text-center">
                  <CheckCircle2 className="w-8 h-8 text-emerald-500/40 mb-2.5" />
      ...
      {/* Status bar */}
      <div className="flex items-center justify-between px-3 h-[22px] border-t border-border/20 bg-card/90 shrink-0 select-none">
        <div className="flex items-center gap-2.5">
          ...
          {hasSelection && (
            <>
              <span className="w-px h-2.5 bg-border/30" />
              <span className="text-[10px] text-primary/70 font-mono font-medium">Selection</span>
            </>
          )}
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-[10px] text-muted-foreground/60 font-mono">SQL</span>
        </div>
      </div>
    </div>
  );
